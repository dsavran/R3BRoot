...
InitStatus R3BGlobalAnalysis::Init()
{
	...
	if(fMappedItems.at(DET_SCI8)){
		...
		fh_sci8_tres_MCFD = new TH1F("sci8_time_res_MCFD", "SCI8 MCFD Time resolution - raw", 3000, 0., 300.);
		fh_sci8_tres_MCFD->GetXaxis()->SetTitle("Time MCFD / ns");
		fh_sci8_tres_TAMEX = new TH1F("sci8_time_res_TAMEX", "SCI8 TAMEX Time resolution -raw ", 800, -4., 4.);  
		...
		fh_tof_sci8 = new TH1F("sci8_tof", "SCI8-LOS ToF",4000,-20,20);
	...
	if(fMappedItems.at(DET_LOS)){
		...
		fh_los_dt_hits = new TH1F("los_dt_hits", "LOS dt multihits",4000,-2000,2000);
		fh_los_dt_hits->GetXaxis()->SetTitle("dt_VFTX between two hits / ns");
		fh_los_dt_hits_l = new TH1F("los_dt_hits_l", "LOS dt multihits leading",4000,-2000,2000);
		fh_los_dt_hits_l->GetXaxis()->SetTitle("dt_LEAD between two hits / ns");
		fh_los_dt_hits_t = new TH1F("los_dt_hits_t", "LOS dt multihits trailing",4000,-2000,2000);
		...
		fh_los_tot_mean = new TH1F("los_tot_mean","LOS mean ToT",3000,0.,300.); 
	...
	for(Int_t ifibcount = 0; ifibcount < NOF_FIB_DET; ifibcount++){
		...
		if(fMappedItems.at(DET_FI_FIRST + ifibcount)) {
			...
			chistTitle=tempTitle.c_str();
			fh_Fib_vs_Events[ifibcount] = new TH2F(chistName, chistTitle, 1000,0,5e6,1100, 0., 1100.);	   
			...
			fh_xpos_Fib[ifibcount]->Draw();
			FibCanvas[ifibcount]->cd(10);
			fh_ypos_Fib[ifibcount]->Draw();        
...
void R3BGlobalAnalysis::Reset_LOS_Histo()
{
...
void R3BGlobalAnalysis::Reset_SCI8_Histo()
{
...
void R3BGlobalAnalysis::Reset_TOFD_Histo()
{
...
void R3BGlobalAnalysis::Reset_FIBERS_Histo()
{
	for(Int_t ifibcount = 0; ifibcount < NOF_FIB_DET; ifibcount++){
		if(fMappedItems.at(DET_FI_FIRST + ifibcount)) {
			...
			fh_xpos_Fib[ifibcount]->Reset();
			fh_ypos_Fib[ifibcount]->Reset();
...
//		tfib = (tMAPMT + tSPMT) / 2.;
		...
		if(ToT>totMax && abs(tof_fib-tof[ifibcount])<20.) {
//		if(ToT>totMax) {
			totMax=ToT;
			iFibMax=iFib;
...
//		tfib = (tMAPMT + tSPMT) / 2.;
				...
				cout << "Det "<<ifibcount<<" totMax=  "<< totMax<<endl;
				cout << "MAPMT ToT "<< hit->GetMAPMTToT_ns() <<" single ToT "<< hit->GetSPMTToT_ns()<<endl;
				cout << "fiber " << iFib << " multi " << m_fib << "Tot Fiber " << ToT << endl;
				cout << "y " << dtime << " tof_fib " << tof_fib << endl;
			}
...
//		if(abs(tof_fib-tof[ifibcount])<20.) 
		if(totMax>0) 
		{
			...
			fh_Fib_vs_Events[ifibcount]->Fill(fNEvents,iFibMax);	
		}
...
//		fh_tofd_TotvsPos[iPlane-1][iBar-1]->Fill((t1l[jm][iPlane-1][iBar-1] - t2l[jm][iPlane-1][iBar-1]),tot1[jm][iPlane-1][iBar-1]);
		if(iBar==16 && iPlane==1)
		fh_tofd_TotvsPos->Fill((t1l[jm][iPlane-1][iBar-1] - t2l[jm][iPlane-1][iBar-1]),sqrt(tot1[jm][iPlane-1][iBar-1]*tot2[jm][iPlane-1][iBar-1]));
	  }
	}
  }
