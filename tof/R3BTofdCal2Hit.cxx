...
InitStatus R3BTofdCal2Hit::Init()
{
    ...
    fHitItemsLos = (TClonesArray*)mgr->GetObject("LosHit");
     if (NULL == fHitItemsLos)
        FairLogger::GetLogger()->Fatal(MESSAGE_ORIGIN, "Branch LosHit not found");
...
*/
		...
		if (NULL == fhTdiffvsQ[iPlane - 1][iBar - 1])
		{
			...
			fhTdiffvsQ[iPlane - 1][iBar - 1] = new TH2F(strName, "", 1000, -10, 10, 1200, 0., 60.);
		}
		if (NULL == fhxy)
		{
			char strName[255];
		...
		if (NULL == fhChargevsPos)
		{
			...
			fhChargevsPos = new TH2F(strName, "", 50, 0, 50, 1000, 0., 100.);
		}
		if (NULL == fhQp12)
		{
			char strName[255];
...
*/	
			...
			if(iPlane==1 || iPlane==3){
				x[iPlane][iBar]=iBar*2.8-22*2.8-2.8*randx;
			}
			else if (iPlane==2 || iPlane==4){
				x[iPlane][iBar]=iBar*2.8-22*2.8+2.8/2.-2.8*randx;	
			}
...
*/		
			...
			fhQ[iPlane-1]->Fill(iBar,Q[iPlane][iBar]);
		...
		for(Int_t j=1;j<=fPaddlesPerPlane;j++){
			if(i == fNofPlanes){
				if(t[i][j]>0. && t[i-1][j]>0. ) {
					...
					Q[i][j] = Q[i][j]+betaCorr(delta);
					fhTdiffvsQ[i-1][j-1]->Fill(delta, Q[i][j]);
				}
			}
			else{
				if(t[i][j]>0. && t[i+1][j]>0. ) {
			...
			if(i==1){
				cout<<"test "<< j << endl;
//				Qp12[j+(j-1)]=(Q[1][j]+Q[2][j-1])/2.;
//				Qp12[j+j]=(Q[1][j]+Q[2][j])/2.;
//				fhQp12->Fill(j+(j-1),Qp12[j+(j-1)]);
//				fhQp12->Fill(j+j,Qp12[j+j]);	
				fhQp12->Fill(1,15);
			}
